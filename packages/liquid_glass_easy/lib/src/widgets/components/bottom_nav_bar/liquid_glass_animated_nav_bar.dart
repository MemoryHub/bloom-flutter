...
class _LiquidGlassAnimatedNavBarState extends State<LiquidGlassAnimatedNavBar>
  ...
  /// or fading out).
  // Bloom keeps the real refracting selection lens alive at rest. The
  // upstream implementation swaps it for a flat painted pill after every
  // travel; that hand-off creates a one-frame hole when taps are repeated
  // quickly and also removes the liquid lens the user expects to keep seeing.
  bool _pillGlassActive = true;
  ...
  static const bool _keepGlassAtRest = true;
  ...
  final LiquidGlassJellySpring _dragJelly = LiquidGlassJellySpring(
    maxVelocity: 6,
    velocityClamp: 60,
  );
  ...
  void _onTick(Duration elapsed) {
    ...
    // 5) Bloom keeps the real refracting lens visible at rest. This avoids
    // the upstream glass -> flat pill hand-off, which can expose a blank
    // frame when the user changes direction during the cross-fade.
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final travelP = travelSpan < 1e-6
            ? 1.0
            : (1.0 - (_travelTarget - _travelPos).abs() / travelSpan).clamp(
                0.0,
                1.0,
              );
              ...
              children: [
                ...
                for (int i = 0; i < widget.outerLenses.length; i++)
                  widget.outerLenses[i].key != null
                      ...
                      : widget.outerLenses[i].copyWith(
                          key: ValueKey('lg-nav-outer-$i'),
                        ),
              ],
              child: widget.outerChild,
            ),
            // Keep tab symbols and labels above the refracting lens. The lens
            // still bends the wallpaper and bar underneath, while the selected
            // glyph reveal stays crisp and readable instead of being sampled
            // repeatedly by the moving optical texture.
            IgnorePointer(
              child: Material(
                ...
                child: LiquidGlassAnimatedBottomNavBarShell(
                  ...
                  // The optical lens may stretch and recoil, but the icon
                  // reveal tracks only the positional spring. This keeps text
                  // and symbols steady as soon as a tab lands.
                  highlightFrac: glassOn ? basePillFrac : null,
                  highlightWidth: glassOn ? glassW : null,
                  highlightHeight: glassOn ? glassH : null,
                ),
              ),
            ),
